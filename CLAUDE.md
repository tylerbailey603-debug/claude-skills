...
**Unreleased (post-v2.11.2) — finance/robinhood-trading (human-gated Robinhood agentic-trading MCP):**

First brokerage-MCP plugin. Bundles Robinhood's official agentic-trading MCP server (`https://agent.robinhood.com/mcp/trading`, launched May 2026 — dedicated agentic account, OAuth handled by Claude Code, per-trade notifications, in-app disconnect) via `.mcp.json`, following the same bundled-remote-MCP pattern as project-management's Atlassian integration, and wraps it in a human-gated trading loop: snapshot → analyze → propose → gate → **human confirms** → review → place → journal → reconcile.

- **3 stdlib tools:** `order_guard.py` (SEC-15c3-5-style pre-trade gate G1–G7 — agentic-account-only, notional/buying-power caps, 20% concentration cap, limit-order + 5% price-band discipline, 5-orders/day rate refusal, blocklist, FINRA-4210 PDT protection; best verdict CLEARED-FOR-HUMAN-CONFIRMATION, exit 0/2/3 — there is deliberately no APPROVED), `portfolio_snapshot_analyzer.py` (snapshot-bridge pattern from saved MCP reads: weights, DOJ/FTC-band HHI, cash %, unrealized P/L, CONCENTRATION/CASH-DRAG/LOSS-RUNNER/REVIEW-WINNER flags — analysis only, never a recommendation), `trade_journal.py` (propose → approve → place ledger; `approve` refuses agent-ish approver names by regex, `place` refuses without a prior named human approval, exit 3/4 refusal codes; atomic writes, chmod 0600).
- **3 references** citing the trading-safety canon (SEC Rule 15c3-5, Knight Capital enforcement order, FINRA 4210/5310/RN-15-09, Robinhood's own guardrail disclosures, MCP spec) + canonical MCP tool list in the atlassian-mcp-tools.md "never invent tool names" style + full order-lifecycle state machine.
- **Hard rules:** explicit per-order in-session human confirmation (blanket pre-approvals refused), `review_equity_order` before every `place_equity_order`, no unattended/scheduled trading, dedicated agentic account only, no investment advice or predictions. `cs-robinhood-trader` agent + `/cs:robinhood` command.
- **Counters:** skills 362 → 363; tools 644 → 647; refs 741 → 744; agents 102 → 103; commands 116 → 117; plugins 88 → 89 (derived via `scripts/derive_counters.py --check`).
...
**Version:** v2.11.2 (+ unreleased productivity coverage expansion + robinhood-trading MCP)
